...
/**
   ...
   */
  public static double[] getCircleContainingThreeObjects(double[] p1, double[] p2, double[] p3) {
	  double offset = Math.pow(p2[0],2) + Math.pow(p2[1],2);
	  double bc =   ( Math.pow(p1[0],2) + Math.pow(p1[1],2) - offset )/2.0;
	  ...
	  if (Math.abs(det) < 1.0E-8) {
		  ...
		  double x = (p1[0] + p2[0] + p3[0]) / 3.0;
		  double y = (p1[1] + p2[1] + p3[1]) / 3.0;
	  ...
	  double centerx =  (bc * (p2[1] - p3[1]) - cd * (p1[1] - p2[1])) * idet;
	  double centery =  (cd * (p1[0] - p2[0]) - bc * (p2[0] - p3[0])) * idet;
   ...
   * @return Three values: center x, center y, and radius.
   */
	public static double[] getCircleContainingObjects(ArrayList<double[]> points) {
