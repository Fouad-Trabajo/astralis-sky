...
public class TimeScale
{
	...
	public static double getTTminusUT1(TimeElement time, ObserverElement obs) throws JPARSECException
	{
		...
		tt = astro.getAstronomicalYear() + (astro.getMonth() - 1 + (astro.getDay() - 1.0) / 30.0) / 12.0;
...
*/
			...
			if (tt < 1955 || approxFuture)
			{
				...
				TTminusUT1 += TimeScale.dynamicalTimeCorrectionForMoonSecularAcceleration(JD_TT);
			}
	...
	throws JPARSECException {
		...
		tt = astro_ut.getAstronomicalYear() + (astro_ut.getMonth() - 1 + (astro_ut.getDay() - 1.0) / 30.0) / 12.0;
	...
	/**
	 * Corrects deltaT for the different value for the Moon secular acceleration adopted as 
	 * correct ({@linkplain Elp2000#MOON_SECULAR_ACCELERATION}) respect the value used by 
	 * Stephenson and Morrison of -26"/centuri^2. This correction is used
	 ...
	 */
	private static double dynamicalTimeCorrectionForMoonSecularAcceleration(double jd)
	{
		...
		double deltaT = -0.91072 * (Constant.MOON_SECULAR_ACCELERATION + 26.0) * cent * cent;
		return deltaT;
	}
